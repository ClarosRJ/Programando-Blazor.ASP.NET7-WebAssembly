...
namespace BlazorPeliculas.Client.Repositorios
{
	public class Repositorio : IRepositorio
	{
		...
		public async Task<HttpResponseWrapper<T>> Get<T>(string url)
		{
			...
			{
				var respuesta = await DesearializarRespuesta<T>(respuestaHTTP, OpcionesPorDefectoJSON);
		...
		public async Task<HttpResponseWrapper<object>> Put<T>(string url, T enviar)
		{
			var enviarJSON = JsonSerializer.Serialize(enviar);
			var enviarContext = new StringContent(enviarJSON, Encoding.UTF8, "application/json");
			var responseHttp = await httpClient.PutAsync(url, enviarContext);
			return new HttpResponseWrapper<object>(null, !responseHttp.IsSuccessStatusCode, responseHttp);
		...
		public async Task<HttpResponseWrapper<TResponse>> Post<T, TResponse>(string url, T enviar)
		{
			...
			{
				var response = await DesearializarRespuesta<TResponse>(responseHttp, OpcionesPorDefectoJSON);
		...
		private async Task<T> DesearializarRespuesta<T>(HttpResponseMessage httpResponse, JsonSerializerOptions jsonSerializerOptions)
		{
